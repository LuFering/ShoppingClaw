...
class OperationLogRepository:
    ...
    async def list_logs(self, limit: int = 50, path_filter: str | None = None) -> list[dict]:
        """获取最近的操作日志（管理员审计查询，按时间倒序）"""
        async with pg_manager.get_async_session_context() as session:
            q = select(OperationLog).order_by(OperationLog.timestamp.desc()).limit(limit)
            if path_filter:
                q = q.where(OperationLog.details.ilike(f"%{path_filter}%"))
