        ...
        <template v-if="sortedChats.length > 0">
          ...
          >
            <div class="conversation-item-main">
              ...
              <p v-if="previewOf(chat.id)" class="conversation-preview">{{ previewOf(chat.id) }}</p>
            </div>
...
// 会话摘要：取该会话最后一条非空消息的前 N 字（标题已是后端自动生成时预览略过）
const previewOf = (chatId) => props.chatPreviews?.[chatId] || ''
