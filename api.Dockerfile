...
RUN set -ex \
    ...
    # (C) 安装必要的系统库 + Node.js 运行时（stdio 型 MCP 子进程需要）
    && apt-get update \
...
# sinataoke_cn MCP（淘宝/拼多多导购，私有包：从本机全局 node_modules 复制，依赖已捆绑）
COPY docker/mcp/sinataoke_cn/ /usr/local/lib/node_modules/@liuliang520500/sinataoke_cn/
RUN printf '#!/bin/sh
exec node /usr/local/lib/node_modules/@liuliang520500/sinataoke_cn/dist/cli.js "$@"
'       > /usr/local/bin/sinataoke_cn && chmod +x /usr/local/bin/sinataoke_cn
